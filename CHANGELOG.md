...
- The bundled skill `.autopus/plugins/autopus-figma/skills/figma-description/SKILL.md` carries the canonical `figma-description` frontmatter (name + figma/피그마/디스크립션/화면 정의/번호 뱃지 triggers) from `.claude/skills/autopus/figma-description.md`, so Claude Code auto-discovers and auto-triggers it on figma/description requests — bundled as the canonical skill, not a divergent fork (REQ-03, HC-2).
- Registered in the local marketplace `.agents/plugins/marketplace.json` mirroring the existing `auto` entry shape (source `{source:"local", path}`, policy, category) (REQ-04, HC-5). `README.md` documents the install path (`claude plugin marketplace add` + `claude plugin install autopus-figma`) and per-environment guidance: Claude Code uses this plugin; Desktop/Cursor rely on SPEC-MCP-001 instructions/prompts + manual MCP config (REQ-09).
- Security boundary: FIGMA_TOKEN is supplied via a sensitive `userConfig` field (secure storage, not committed); the manifest and bundled skill carry no FIGMA_TOKEN literal and no per-session channel secret (REQ-06, REQ-08, HC-1 — verified by negative grep for `figd_`/32-hex/token literals).
- Packaging-only (HC-4): no new `.ts` source. The bundled MCP server is the same stdio entry SPEC-MCP-001 enhances; the one-install value completes together with that sibling (HC-3). Review: PASS (23/23 checklist, claude provider; codex timed out — degraded), 3 minor/suggestion findings recorded out-of-scope.

### Added — Frame-description workflow bundled into the MCP server: instructions + prompts capability (SPEC-MCP-001)
Connecting the autopus-figma MCP server previously exposed only a tool list and a one-line meta description, so no client (Claude Desktop, Cursor, Claude Code) learned *when or in what order* to drive the frame-description workflow. This release makes the MCP server runtime itself deliver the procedure over two paths: connect-time instructions and an explicit prompts capability.
- New single source-of-truth module `src/daemon/figma-workflow-guidance.ts` exports `FRAME_DESCRIPTION_WORKFLOW` (the ordered get_active_selection/get_stale_frames → dryRun → approve → apply → undo guidance, derived from `.claude/skills/autopus/figma-description.md`) and a `renderWorkflowInstructions()` helper. The stdio entry, the http session, and the prompts handler all render from this one constant — no divergent inline strings.
- Both transports now advertise `capabilities.prompts` and register ListPrompts/GetPrompt handlers (`src/daemon/mcp-stdio-prompt-handlers.ts`), exposing the workflow as a `/mcp__autopus-figma__generate_frame_descriptions` slash command. GetPrompt returns a user-role text message embedding the ordered tool names; the active description language (live `descriptionLanguage()` getter, default ko) is stated when wired.
- Security boundary preserved: the per-session channel secret never enters prompt payloads (REQ-08 negative assertion in tests); it remains only in the trusted stdio initialize instructions (C-1). The additive prompts surface leaves the tools/resources wire surface byte-equal (SPEC-FIGMA-009 INV-W4 baseline; REQ-09).
- HC-5: the shared workflow body names only tools registered on both transports; the stdio-only `get_description_language` and the secret are appended outside the shared constant so the http transport never advertises a tool it does not register.
- Verified: 9 new unit tests across `tests/unit/mcp-prompts-surface.test.ts` and `tests/unit/mcp-prompts-edge.test.ts` (in-memory client/server pair) green; `npm run lint` (tsc --noEmit) clean. Claude Code plugin bundle / skill auto-trigger is the sibling SPEC-MCP-002.
