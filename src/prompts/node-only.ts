...
const SCHEMA_HINT = `Output JSON shape (illustrative):
{
  ...
  "states": ["<one tight line per state: 'state — trigger -> UI/data/motion expectation', e.g. '검증 중 — 제출 직후 -> 버튼 비활성화 + 스피너로 중복 제출 차단'>"],
  "edge_cases": ["<one tight line per case: risk -> expected handling. Cover interaction, QA branch, permission/error, accessibility (text alternative / keyboard + focus order / color-independence / contrast / target size / screen-reader label), reduced-motion, or a [CANNOT_INFER] policy gap stated as a concrete risk>"],
