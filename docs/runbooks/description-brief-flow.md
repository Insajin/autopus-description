...
1. Generate a brief template:

   ```bash
   node dist/src/cli/generate-descriptions.js --init-project-brief=.autopus/project/description-brief.json
   ```

2. Ask the stakeholder to fill the template. Unknown policies should stay empty
   or move to `open_questions`; the generator must not invent them.

3. Run generation with the brief:

   ```bash
   node dist/src/cli/generate-descriptions.js <input-dir> <output-manifest> --project-brief=.autopus/project/description-brief.json
   ```
...
- `states`: UI, data, permission, and error states.
- `edge_cases`: QA branches and unresolved implementation questions.
